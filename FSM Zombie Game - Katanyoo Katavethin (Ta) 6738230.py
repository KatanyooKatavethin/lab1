...
#Human image
hM = pygame.image.load("assets/Human.png")
...
zombie_sprite_sheet = pygame.image.load("assets/Zombie.png").convert_alpha()
zombie_idle_animation = [zombie_sprite_sheet.subsurface(pygame.Rect(x * 100, 0, 100, 210)) for x in range(6)]
zombie_attack_animation = [zombie_sprite_sheet.subsurface(pygame.Rect(x * 100, 210, 100, 210)) for x in range(6)]
zombie_chase_animation = [zombie_sprite_sheet.subsurface(pygame.Rect(x * 100, 420, 100, 210)) for x in range(6)]
zombie_eat_animation = [zombie_sprite_sheet.subsurface(pygame.Rect(x * 100, 630, 100, 210)) for x in range(6)]

#zombie_frames = zombie_idle_animation
...
class IdelState(State):
    def enter(self, agent):
        agent.current_animation = zombie_idle_animation
        pass
...
class EatState(State):
    def enter(self, agent):
        agent.current_animation = zombie_eat_animation
        agent.velocity *= 0
...
class ChaseState(State):
    def enter(self, agent):
        agent.current_animation = zombie_chase_animation
        pass
...
class AtkState(State):
    def enter(self, agent):
        #agent.velocity *= 0
        agent.current_animation = zombie_attack_animation
...
class Agent:
    def __init__(self):
        ...
        self.current_animation = zombie_idle_animation
    ...
    def draw(self, screen):
        ...
        if self.state_machine.curret_state == 'idel':
        ...
        elif self.state_machine.curret_state == 'eat':
            pygame.draw.circle(screen, (54, 1, 63 ), self.position, 10)
...
def main():
    ...
    while running:
        ...
        screen.blit(hM, ((target.x -18), (target.y -15)))
